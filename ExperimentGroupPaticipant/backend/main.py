...
def batch_generate_mock_jobs(n):
    ...
    for _ in range(n):
        ...
        job = {"title": title, "company": company, "location": location, "salary": salary}
        jobs.append(job)
    return jobs
...
def get_jobs(min_salary: int = Query(0), companies: List[str] = Query([]), locations: List[str] = Query([])): 
    ...
    results = list(jobs_collection.find(query, {"_id": 0}))
...
def get_stats():
    # stats avg salary
    avg_salary_pipeline = [
        {"$group": {"_id": "$title", "avg_salary": {"$avg": "$salary"}}} #group and title needs to change to different names
    ]
    avg_salary_result = list(jobs_collection.aggregate(avg_salary_pipeline))
    
    # stats avg jobs count
    skill_counts_pipeline = [
        {"$group": {"_id": "$title", "count": {"$sum": 1}}}, #group and title needs to change to different names 
        {"$sort": {"count": -1}}
    ]
